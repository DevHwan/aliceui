#include "pch.h"
#include "AUITemplateProcessor.h"
...
void AUITemplateProcessor::SetPool( const std::shared_ptr< AUITemplatePool >& pPool )
{
    if ( IsLoaded() )
    {
...
void AUITemplateProcessor::Clear()
{
    if ( IsLoaded() )
    {
...
bool AUITemplateProcessor::Insert( const std::wstring& tag, const std::shared_ptr< AUITemplate >& pTemplate )
{
    if ( IsLoaded() )
    {
        AUIAssertFailReason("Unload first");
...
bool AUITemplateProcessor::Remove( const std::wstring& tag )
{
    if ( IsLoaded() )
    {
...
void AUITemplateProcessor::SetRootWidget( const std::shared_ptr< AUILinearLayoutWidget >& pRootWidget )
{
    if ( IsLoaded() )
    {
        AUIAssertFailReason("Unload first");
...
void AUITemplateProcessor::SetRootWidget( AUILinearLayoutWidget* const pRootWidget )
{
    if ( nullptr == pRootWidget )
    {
        AUIAssertFailReason("Invalid widget");
...
void AUITemplateProcessor::SetRootTags( const std::vector< std::wstring >& arrRootTags )
{
    if ( IsLoaded() )
    {
        AUIAssertFailReason("Unload first");
...
bool AUITemplateProcessor::Load()
{
    if ( IsLoaded() )
    {
        AUIAssertFailReason("Unload first");
...
bool AUITemplateProcessor::Unload()
{
    if ( false == IsLoaded() )
    {
        return true;
...
void AUITemplateProcessor::UpdateParam( const std::wstring& tag, const AUITemplParam& param )
{
    if ( false == IsLoaded() )
    {
        AUIAssertFail();
...
std::shared_ptr< AUIWidget > AUITemplateProcessor::GetTagWidget( const std::wstring& tag ) const
{
...
bool AUITemplateProcessor::Has( const std::wstring& tag ) const
{
    return m_pPool->Has( tag );
}

bool AUITemplateProcessor::TrPrepare()
{
    if ( false == IsLoaded() )
    {
...
bool AUITemplateProcessor::TrCommit()
{
...
bool AUITemplateProcessor::TrRollback()
{
    if ( false == IsTrPrepared() )
    {
        AUIAssertFailReason("Prepare TR first");
...
void AUITemplateProcessor::TrInsert( const std::wstring& toTag, const std::wstring& targetTag, const std::wstring& text, const std::shared_ptr< AUIDrawable>& pImage )
{
    m_Transactions.emplace_back( Tr::Insert_Cmd, toTag, targetTag, text, pImage );
}

void AUITemplateProcessor::TrInsert( const std::wstring& toTag, const std::shared_ptr< AUIWidget >& pWidget, const std::wstring& text, const std::shared_ptr< AUIDrawable >& pImage )
{
    m_Transactions.emplace_back( Tr::Insert_Cmd, toTag, pWidget, text, pImage );
}

void AUITemplateProcessor::TrRemove( const std::wstring& toTag, const std::wstring& targetTag )
{
    m_Transactions.emplace_back( Tr::Remove_Cmd, toTag, targetTag );
}

void AUITemplateProcessor::TrRemove( const std::wstring& toTag, const std::shared_ptr< AUIWidget >& pWidget )
{
    m_Transactions.emplace_back( Tr::Remove_Cmd, toTag, pWidget );
}

void AUITemplateProcessor::TrClear( const std::wstring& toTag )
{
    m_Transactions.emplace_back( Tr::Clear_Cmd, toTag, L"" );
}


bool AUITemplateProcessor::TrOptimize()
{
    if ( false == IsTrPrepared() )
    {
        AUIAssertFailReason("Prepare TR first");
...
std::vector< std::wstring > AUITemplateProcessor::GetAllTag() const
{
